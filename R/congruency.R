...
check_congruency <- function(data_filename, metadata_filename, data_template='templates/wqp_template.csv', metadata_template = 'templates/wqp_ncrnwater_metadata_template.csv') {
  ...
  files <- list(
    ...
    ,'template_files' = list(
      'data' = read.csv(data_template)
      'metadata' = read.csv(metadata_template)
    )
  )
  # 
...
check_congruency_columns_match <- function(files) {
  
}

check_congruency_files_exist <- function(files) {
  tmp <- list(
    'msgs' = c()
    ,problems = 0
  )
...
check_congruency_files_are_csvs <- function(files) {
  tmp <- list(
    'msgs' = c()
    ,problems = 0
  )
